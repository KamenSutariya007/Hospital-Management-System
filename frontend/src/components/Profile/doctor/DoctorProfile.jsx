...
function DoctorProfile() {
  ...
  // Handle both users table (userName, phoneNumber, dateOfBirth) and doctors table (name, phoneno, dob) field names
  const [name, setName] = useState(initialUser?.userName || initialUser?.name || "");
  const [mobileNumber, setMobileNumber] = useState(initialUser?.phoneNumber || initialUser?.phoneno || "");
  ...
  const [dateOfBirth, setdateofBirth] = useState(formatDateOfBirth(initialUser?.dateOfBirth || initialUser?.dob));
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        ...
        if (user && user._id) {
          // Fetch latest data from users table (not doctors table)
          try {
            ...
            console.warn("Could not fetch from users API, trying doctors API:", apiError);
            // Try doctors table as fallback
            try {
              const doctorResponse = await axios.get(`http://localhost:5000/doctor/get-doctor/${user._id}`);
              if (doctorResponse.data) {
                const latestUser = doctorResponse.data;
                localStorage.setItem("user", JSON.stringify(latestUser));
                setuserData(latestUser);
              ...
              console.warn("Could not fetch from doctors API either:", doctorError);
            }
          }
          
          // Fallback to localStorage data if API fails
          console.log("Using localStorage data:", user);
          console.log("Phone from localStorage:", user.phoneNumber || user.phoneno);
          console.log("DOB from localStorage:", user.dateOfBirth || user.dob);
          
          setuserData(user);
          // Handle both users table and doctors table field names
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      // Check if user is from users table (has userName) or doctors table (has name)
        ...
        axios
          .put("http://localhost:5000/user/profile-update", {
            ...
            updatedProfile: {
              ...
              phoneNumber: mobileNumber, // Users table uses phoneNumber
              address: {
                street: address,
                city: city,
                state: state,
              ...
              gender: gender,
              dateOfBirth: dateOfBirth, // Users table uses dateOfBirth
            },
          })
          .then((res) => {
